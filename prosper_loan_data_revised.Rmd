...
```{r echo=FALSE, Univariate_Plots, fig.width=12, fig.height=8}
dim(pld)
...
We have log transformed the Y axis because the maximum is few orders of magnitude greater than the minimum values
...
There are many Occupations in the dataset. In order to form our understanding around this attribute we would group the related occupations so that we have only a handful of distinct values to concentrate on which in turn will help us understand the data better. The name of the new column derived will be "Occupation_Derived"
...
pldca$HomeOwnership <- ifelse(pldca$IsBorrowerHomeowner == 'True',
                              'OwnHome','RentedHome')
# New Variable Derived for Credit Grade
...
# New Variable Derived for Occupation
pldca$Occupation_Derived <- as.character(pldca$Occupation)
pldca$Occupation_Derived <- ifelse(pldca$Occupation_Derived %in% 
                                     c("Principal",
                                      "Professor",
                                      "Teacher",
                                      "Teacher\'s Aide"
                                      ), "Teaching",
                                   pldca$Occupation_Derived)
pldca$Occupation_Derived <- ifelse(pldca$Occupation_Derived %in% 
                                     c("Architect",
                                      "Construction",
                                      "Landscaping"
...
pldca$Occupation_Derived <- ifelse(pldca$Occupation_Derived %in% 
                                   ...
                                   pldca$Occupation_Derived)
pldca$Occupation_Derived <- ifelse(pldca$Occupation_Derived %in% 
                                     c("Police Officer/Correction Officer",
                                       "Fireman",
                                       "Civil Service"
                                      ), "Civil Service",
                                   pldca$Occupation_Derived)
...
Occupation_Derived - This field is derived from Occupation by grouping similar Occupations under one head   
...
```{r echo=FALSE, Histogram_of_Occupation, fig.width=12, fig.height=8 }
# Exploring the Occupation variable
# Log Transforming the Y-axis
ggplot(data=pldca,aes(x=Occupation_Derived))+
  geom_histogram()+
  scale_y_log10()+
  xlab("Occupation")+
  theme(axis.text.x = element_text(face="bold",  
...
More than 30% of the people are in Other category and almost 15% have been classified as Professional. Besides these, Clerical, Sales and Computer categories are more as compared to other professions
...
Most of the cases are Employed and Full-time. We have log transformed the Y axis because the maximum is few orders of magnitude greater than the minimum values
...
ggplot(pld,aes(x=LoanOriginalAmount))+
  ...
  scale_x_continuous(limits=c(0,35000),breaks=seq(0,35000,1500))+
  theme(axis.text.x = element_text(face="bold",  
                           size=10, angle=90),
          axis.text.y = element_text(face="bold",  
                           size=10))
...
The Gross Principal Loss is zero for more than the first 3 quantiles. Hence we will ignores cases with Gross Principal Loss Zero. We see that there are more cases having Gross Principal Loss upto $ 4000 and there is a gradual decline in the cases for higher amounts. Also we see that after 15000 the number of cases are very few.
...
Almost 1/3rd of the cases never had delinquent trades. Hence we ignore cases which had delinquent trade less than one percent to get a more better understanding of the trades which were delinquent atleast 1 percent. Here we see a gradual increase from 0 to 0.99 peaking at around 0.95
...
I created 3 new variables CreditGrade_Derived, HomeOwnership and Occupation_Derived   
...
Occupation_Derived - There are many Occupations in the dataset. In order to form our understanding around this attribute we have grouped the related occupations so that we have only a handful of distinct values to concentrate on. This in turn will help us understand the data better. we have derived this field from Occupation field by grouping similar Occupations under one head. 

### Of the features you investigated, were there any unusual distributions? Did you perform any operations on the data to tidy, adjust, or change the form of the data? If so, why did you do this?

For the following features the maximum is a few orders of magnitude greater than the minimum values. Hence we log transformed the Y axis in order to depict the information in the chart more visible   
BorrowerState   
CreditGrade_Derived   
LoanStatus   
Occupation_Derived   
...
The fields LP_GrossPrincipalLoss has zero for many cases and TradesNeverDelinquent_percentage has 100 for many cases. So,  we have ignored these cases and have plotted the histogram for these fields to better understand their distribution

# Bivariate Plots Section

```{r echo=FALSE, Bivariate_Plots,fig.width=12, fig.height=8}
...
```{r echo=FALSE, Histogram_BorrowerRate_facetwrap_CreditGrade,fig.width=12, fig.height=8}
...
```{r echo=FALSE, LenderYield_CreditGrade,fig.width=12, fig.height=8}
...
```{r echo=FALSE, LoanStatus_CreditGrade,fig.width=12, fig.height=8}
# Log Transforming the Y-axis
...
```{r echo=FALSE, Homeownership_TradesNeverDelinquentpercent,fig.width=12, fig.height=8}
...
```{r echo=FALSE, Homeownership_DebtToIncomeRatio,fig.width=12, fig.height=8}
...
```{r echo=FALSE, scatterplot_occupation_TradesNeverDelinquent, fig.width=12, fig.height=8}


p9 <- ggplot(data=subset(pldca,TradesNeverDelinquent_percentage == 1),
       aes(x=Occupation_Derived),stat=bin)+
  geom_bar()+
  scale_y_log10()+
  ylab("Number of Trades never Deliquent")+
...
p10 <- ggplot(data=subset(pldca,TradesNeverDelinquent_percentage < 1),
       aes(x=Occupation_Derived,y=TradesNeverDelinquent_percentage))+
  geom_boxplot(aes(color=Occupation_Derived))+
  xlab("Occupation")+
  ylab("Trades never Deliquent Ratio")+
  theme(axis.text.x = element_text(face="bold",  
                           size=10, angle=90),
          axis.text.y = element_text(face="bold",  
                           size=10),
...
We have log transformed the Y axis for the bar chart of Occupation for Trades Never Delinquent Percentage equal to 1 as there is a wide difference between the maximum and minimum Values

The Occupations "Aviation" and "Car Dealer" have more Trade Delinquencies as compared to other Trades with their mean around 25% (The mean of trades not Delinquent is around 75% for these 2 groups implying 25% Delinquency). For these 2 groups the number of Trades never Delinquent is also very low. Followed by these 2 groups are the groups "Home Maker", "Religious" and "Realtor" which have higher Delinquency with mean around 20% and these groups also have very low number of Trades which were never Delinquent.

The Occupations "Other", "Clerical", "Computer", "Professional" and "Sales" have more number of non Delinquent Trades as compared to other Occupations
...
ggplot(data=subset(pldca,TradesNeverDelinquent_percentage == 1 &
                     DebtToIncomeRatio <= 1),
       aes(x=DebtToIncomeRatio),stat=bin)+
  geom_bar()

ggplot(data=subset(pldca,TradesNeverDelinquent_percentage != 1),
       aes(x=TradesNeverDelinquent_percentage, y=DebtToIncomeRatio))+
  ylim(0, 1)+
  geom_point(alpha=0.05)+
  geom_smooth(method = 'lm', color= 'red')
...
2 Plots have been plotted to understand the relation between the Debt to Income Ratio and Trades Never Delinquent Percentage   

The first plot is a bar plot of Debt to Income Ratio for Trades which have never been Delinquent. Most of the cases are for Debt to Income Ratio less than 0.5. The number of cases gradually increases from 0 and peaks at around 0.25 and then gradually decreases   
   
The Second Plot is a scatter plot between Stated Monthly Income and Trades Never Delinquent Percentage for cases having Delinquent Trades   
...
2 Plots have been plotted to understand the relation between the Stated Monthly Income and Trades Never Delinquent Percentage   

The first plot is a bar plot of Stated Monthly Income for Trades which have never been Delinquent. Most of the cases are for Monthly Income less than 10000 which is intuitive as most of the Loan cases also have Monthly Income less than 10000   
...
The Occupations "Aviation" and "Car Dealer" have more Trade Delinquencies as compared to other Trades with their mean around 25%. For these 2 groups the number of Trades never Delinquent is also very low. Followed by these 2 groups are the groups "Home Maker", "Religious" and "Realtor" which have higher Delinquency with mean around 20% and these groups also have very low number of Trades which were never Delinquent.
...
```{r echo=FALSE, Multivariate_Plots, fig.width=12, fig.height=8}
...
We have log transformed the Yaxis
...
```{r echo=FALSE, ClosedLoan_CreditGrade_HomeOwnership_Loanstatus, fig.width=12, fig.height=8}
...
```{r echo=FALSE, ClosedLoan_CreditGrade_HomeOwnership_PrincipalLossPctg, fig.width=12, fig.height=8}
...
ggplot(data=subset(pldca,CreditGrade_Derived != 'NA' 
  ...
  geom_histogram(binwidth=10,aes(fill=CreditGrade_Derived))+
...
```{r echo=FALSE, Summarize_using_dplyr, fig.width=12, fig.height=8}
...
```{r echo=FALSE, dataset_wide_format, fig.width=12, fig.height=8}
...
There are certain occupations which have more Trade Delinquencies as compared to others. The Occupations "Aviation" and "Car Dealer" have more Trade Delinquencies as compared to other Trades with their mean around 25%. Followed by these 2 groups are the groups "Home Maker", "Religious" and "Realtor" which have higher Delinquency with mean around 20%. All these groups also have very low number of Trades which were never Delinquent. The Occupations "Other", "Clerical", "Computer", "Professional" and "Sales" have more number of non Delinquent Trades as compared to other Occupations
